...
module.exports = {
	...
	defaultConfigDetails: {
		...
		apiKey: {
			label: "SWGT API key (On your SWGT profile page)",
			type: "input",
		},
		siteURL: {
			label: "SWGT API URL  (On your SWGT profile page)",
			type: "input",
		},
	},
	pluginName,
	pluginDescription:
		"For SWGT Patreon subscribers to automatically " +
	...
	init(proxy, config) {
		...
		proxy.log({
			...
			message:
				...
				listenToSWGTCommands.toString().replace(/,/g, ", ") +
				"<br><br>" +
				listenTo3MDCCommands.toString().replace(/,/g, ", "),
		});
		//Attach SWGT events
	...
	processRequest(command, proxy, config, req, resp, cache) {
		...
		if (resp["command"] == "HubUserLogin") {
			...
			for (var i in requiredHubUserLoginElements) {
				...
				pruned[requiredHubUserLoginElements[i]] = JSON.parse(
					JSON.stringify(resp[requiredHubUserLoginElements[i]])
				);
			}
			...
			try {
				...
				for (var k = wizardBattles.length - 1; k >= 0; k--) {
					if (wizardBattles[k].wizard_id == req["wizard_id"]) {
						//update rating id
						wizardBattles[k].guild_rating_id =
							resp["guildwar_match_info"]["guild_rating_id"];
				...
				proxy.log({
					...
					source: "plugin",
					name: this.pluginName,
					message: `${resp["command"]}-${e.message}`,
				});
			}
		}
				...
				for (var k = wizardBattles.length - 1; k >= 0; k--) {
					if (wizardBattles[k].wizard_id == req["wizard_id"]) {
						...
						for (var wizard in resp["wizard_info_list"]) {
							if (
								resp["wizard_info_list"][wizard].wizard_id == req["wizard_id"]
							) {
								wizardBattles[k].guild_id =
									resp["wizard_info_list"][wizard].guild_id;
							}
						}
						wizardBattles[k].sendBattles = [];
						wizardFound = true;
			...
			try {
				for (var i = 0; i < 2; i++) {
					...
					for (var j = 0; j < 3; j++) {
						try {
							//Offense Mons
							battle.counter.units.push(
								resp.guildwar_my_unit_list[i][j].unit_master_id
							);

							//Defense Mons
							battle.defense.units.push(
								resp.guildwar_opp_unit_list[i][j].unit_info.unit_master_id
							);
						} catch (e) {}
					}
			...
			try {
				...
				for (var j = 0; j < 3; j++) {
					try {
						//Offense Mons
						battle.counter.units.push(
							resp.guildsiege_my_unit_list[j].unit_master_id
						);

						//Defense Mons
						battle.defense.units.push(
							resp.guildsiege_opp_unit_list[j].unit_info.unit_master_id
						);
					} catch (e) {}
				}
				//match up wizard id and push the battle
		...
		if (req["command"] == "BattleGuildWarResult") {
			...
			try {
				...
				for (var wizard in wizardBattles) {
					for (
						var k = wizardBattles[wizard].sendBattles.length - 1;
						k >= 0;
						k--
					) {
						if (
							wizardBattles[wizard].sendBattles[k].battleKey ==
							req["battle_key"]
						) {
							wizardBattles[wizard].sendBattles[k].win_lose =
								req["win_lose_list"][j];
							wizardBattles[wizard].sendBattles[k].battleDateTime =
								resp.tvalue - j;
							j--;
							...
							if (
								sendResp.defense.units.length == 3 &&
								sendResp.counter.units.length == 3 &&
								sendResp.battleRank >= 4000
							) {
								...
								proxy.log({
									...
									source: "plugin",
									name: this.pluginName,
									message: `GW Battle End Processed ${k}`,
								});
							}
						}
					}
				...
				proxy.log({
					...
					source: "plugin",
					name: this.pluginName,
					message: `GW Battle End Error ${e.message}`,
				});
			}
			if (j == 1) {
				j = 0;
		...
		if (req["command"] == "BattleGuildSiegeResult") {
			...
			try {
						...
						if (
							...
							if (
								...
								proxy.log({
									...
									source: "plugin",
									name: this.pluginName,
									message: `Siege Battle End Processed ${k}`,
								});
							}
						}
					}
				...
				proxy.log({
					...
					source: "plugin",
					name: this.pluginName,
					message: `Siege Battle End Error ${e.message}`,
				});
			}
			if (j == 1) {
				j = 0;
	...
	processSWGTHistoryRequest(command, proxy, config, req, resp, cache) {
		...
		if (
			resp["command"] == "GetGuildSiegeBaseDefenseUnitList" ||
			resp["command"] == "GetGuildSiegeBaseDefenseUnitListPreset" ||
			resp["command"] == "GetGuildSiegeDefenseDeckByWizardId"
		) {
			//If wizard id and rating doesn't exist in wizardBattles[] then push to it
			try {
				defenseInfo = {};
				...
				for (var deck in resp["defense_deck_list"]) {
					...
					for (var defenseUnit in resp["defense_unit_list"]) {
						if (
							defenseInfo.deck_id ==
								resp["defense_unit_list"][defenseUnit].deck_id &&
							resp["defense_unit_list"][defenseUnit].pos_id == 1 &&
							resp["defense_unit_list"][defenseUnit].hasOwnProperty("unit_info")
						) {
							defenseInfo.uniqueMon1 =
								resp["defense_unit_list"][defenseUnit].unit_info.unit_id;
							defenseInfo.mon1 =
								resp["defense_unit_list"][defenseUnit].unit_info.unit_master_id;
							unitCount++;
						}
						if (
							defenseInfo.deck_id ==
								resp["defense_unit_list"][defenseUnit].deck_id &&
							resp["defense_unit_list"][defenseUnit].pos_id == 2 &&
							resp["defense_unit_list"][defenseUnit].hasOwnProperty("unit_info")
						) {
							defenseInfo.uniqueMon2 =
								resp["defense_unit_list"][defenseUnit].unit_info.unit_id;
							defenseInfo.mon2 =
								resp["defense_unit_list"][defenseUnit].unit_info.unit_master_id;
							unitCount++;
						...
						if (
							defenseInfo.deck_id ==
								resp["defense_unit_list"][defenseUnit].deck_id &&
							resp["defense_unit_list"][defenseUnit].pos_id == 3 &&
							resp["defense_unit_list"][defenseUnit].hasOwnProperty("unit_info")
						) {
							defenseInfo.uniqueMon3 =
								resp["defense_unit_list"][defenseUnit].unit_info.unit_id;
							defenseInfo.mon3 =
								resp["defense_unit_list"][defenseUnit].unit_info.unit_master_id;
							unitCount++;
					...
					if (unitCount == 3) {
						...
						defenseInfo.deckPrimaryKey =
							defenseInfo.wizard_id.toString() +
							"_" +
							defenseInfo.uniqueMon1.toString() +
							"_" +
							defenseInfo.uniqueMon2.toString() +
							"_" +
							defenseInfo.uniqueMon3.toString();
			...
			try {
				...
				for (var k = tempDefenseDeckInfo.length - 1; k >= 0; k--) {
					if (tempDefenseDeckInfo[k].deck_id == req["target_deck_id"]) {
						deckIDPrimaryKey =
							tempDefenseDeckInfo[k].wizard_id.toString() +
							"_" +
							tempDefenseDeckInfo[k].uniqueMon1.toString() +
							"_" +
							tempDefenseDeckInfo[k].uniqueMon2.toString() +
							"_" +
							tempDefenseDeckInfo[k].uniqueMon3.toString();
					}
				}
				for (var siegewar in resp["log_list"]) {
					for (var battleLog in resp["log_list"][siegewar].battle_log_list) {
						...
						deckLogValues.wizard_id =
							resp["log_list"][siegewar].battle_log_list[battleLog].wizard_id;
						deckLogValues.wizard_name =
							resp["log_list"][siegewar].battle_log_list[battleLog].wizard_name;
						deckLogValues.opp_wizard_id =
							resp["log_list"][siegewar].battle_log_list[
								battleLog
							].opp_wizard_id;
						deckLogValues.opp_wizard_name =
							resp["log_list"][siegewar].battle_log_list[
								battleLog
							].opp_wizard_name;
						deckLogValues.win_lose =
							resp["log_list"][siegewar].battle_log_list[battleLog].win_lose;
						deckLogValues.log_type =
							resp["log_list"][siegewar].battle_log_list[battleLog].log_type;
						deckLogValues.log_timestamp =
							resp["log_list"][siegewar].battle_log_list[
								battleLog
							].log_timestamp;
						deckLogValues.linkPrimaryKey =
							deckLogValues.wizard_id +
							"_" +
							deckLogValues.opp_wizard_id +
							"_" +
	...
	uploadToWebService(proxy, config, req, resp, endpointType) {
		...
		var endpoint = "/api/v1";
		if ("3MDC" == endpointType) endpoint = "/api/3mdc/v1";

		let options = {
			method: "post",
			uri:
				config.Config.Plugins[pluginName].siteURL +
				endpoint +
				"?apiKey=" +
				config.Config.Plugins[pluginName].apiKey,
			json: true,
			body: resp,
		};
		...
		request(options, (error, response) => {
			if (error) {
				proxy.log({
					...
					source: "plugin",
					name: this.pluginName,
					message: `Failed to connect to ${config.Config.Plugins[pluginName].siteURL}`,
				});
				return;
			}
			...
			if (response.statusCode === 200) {
				proxy.log({
					...
					source: "plugin",
					name: this.pluginName,
					message: `Successfully connected to ${config.Config.Plugins[pluginName].siteURL}`,
				});
	...
	writeToFile(proxy, req, resp, prefix) {
		...
		let filename =
			prefix + "-" + resp["command"] + "-" + new Date().getTime() + ".json";
		...
		proxy.log({
			...
			source: "plugin",
			name: this.pluginName,
			message: "Saved data to ".concat(filename),
		});
	},
};
