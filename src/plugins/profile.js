...
module.exports = {
		...
		proxy.on("getUnitStorageList", (req, resp) => {
			if (this.temp[req.wizard_id]) {
				this.temp[req.wizard_id] = {
					...this.temp[req.wizard_id],
					unit_storage_list: resp.unit_storage_list,
				};
	...
	writeProfileToFile(proxy, wizardID) {
		...
		const filePath = path.join(app.getPath("desktop"), `${app.name} Files`);
		let outFile = fs.createWriteStream(path.join(filePath, filename), {
			flags: "w",
			autoClose: true,
		});
		...
		proxy.commandList.unshift({
			command: `계정 정보가 ${filename}에 저장되었습니다.`,
			time: nowTime,
		});
		proxy.log({
			type: "success",
			source: "plugin",
			name: this.pluginName,
			message: "Saved profile data to ".concat(filename),
		});
	},
	checkData(data) {
		// Sometimes com2us doesn't include all the required data in the request
	...
	sortUserData(data) {
		...
		data.unit_list = data.unit_list.sort((a, b) =>
			cmp(
				[
					cmp(
						a.building_id === storageID ? 1 : 0,
						b.building_id === storageID ? 1 : 0
					),
					-cmp(a.class, b.class),
		...
		data.rune_craft_item_list = data.rune_craft_item_list.sort((a, b) =>
			cmp(
				[
					cmp(a.craft_type, b.craft_type),
					cmp(a.craft_item_id, b.craft_item_id),
				],
