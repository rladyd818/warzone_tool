...
module.exports = {
	...
	logDungeon(proxy, req, resp) {
		...
		if (winOrLost === "Win") {
			...
			if (reward.crate && reward.crate.rune) {
				this.saveAction(proxy, req.wizard_id, resp.tvalue, "new_rune", {
			...
			if (rewards) {
				rewards.forEach((reward) => {
					if (reward.type === 8) {
						...
						this.saveAction(proxy, req.wizard_id, resp.tvalue, "new_artifact", {
							artifact: reward.info,
						});
					}
				});
			}
	...
	logCraftRune(proxy, req, resp) {
		if (resp.reward && resp.reward.crate && resp.reward.crate.runes) {
			this.saveAction(proxy, req.wizard_id, resp.tvalue, "new_rune", {
				rune: resp.reward.crate.runes[0],
			});
		}
	},

	logReappraiseRune(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "reappraise_rune", {
			rune: resp.rune,
		});
	},

	logAmplifyRune(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "amplify_rune", {
			rune_id: req.rune_id,
			craft_id: req.craft_item_id,
			rune: resp.rune,
		});
	},

	logConvertRune(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "convert_rune", {
			rune_id: req.rune_id,
			craft_id: req.craft_item_id,
			rune: resp.rune,
		});
	...
	logEquipRune(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "equip_rune", {
			rune_id: req.rune_id,
			mob_id: req.unit_id,
		});
	},
	...
	logUnequipRune(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "unequip_rune", {
			rune_id: req.rune_id,
		});
	},
	...
	logUpgradeRune(proxy, req, resp) {
		...
		if (newLevel > originalLevel) {
			this.saveAction(proxy, req.wizard_id, resp.tvalue, "upgrade_rune", {
				rune: resp.rune,
			});
		}
	},
	...
	logSellRune(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "sell_rune", {
			rune_id_list: req.rune_id_list,
		});
	},

	logSellCraft(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "sell_craft", {
			craft_id_list: req.craft_item_id_list,
		});
	},

	logUpgradeArtifact(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "upgrade_artifact", {
	...
	logSellArtifact(proxy, req, resp) {
		this.saveAction(proxy, req.wizard_id, resp.tvalue, "sell_artifact", {
			artifact_id_list: req.artifact_ids,
		});
	},
			...
			for (const rewardID in resp.battle_reward_list) {
				if (wizardId === resp.battle_reward_list[rewardID].wizard_id) {
					...
					if (reward.item_master_type === 27) {
						...
						this.saveAction(proxy, wizardId, resp.tvalue, "new_craft", {
							craft: changestone,
						});
						break;
					}
